...
@onready var capacity_label = $NinePatchRect/CapacityLabel
...
func _ready():
	add_to_group("depot_ui")
	show_depot(DepotManager.depot_items)
	_set_layout()
	visible = false
...
func add_item_to_slot(index: int, data):
	...
	if index >= 0 and index < grid.get_child_count():
		...
		DepotManager.depot_items[index] = slot.item_data.duplicate()
	...
	for i in grid.get_child_count():
		...
		if slot.item_data.has("id"):
			if item_id == slot.item_data["id"]:
				slot.item_data["count"] += 1
				slot._update_display()
				DepotManager.depot_items[i]["count"] += 1
				return true
	return false
...
func _set_layout():
	...
	# 容量标签
	capacity_label.anchor_left = 1.0
	capacity_label.anchor_right = 1.0
	capacity_label.offset_left = -120  # 从右侧定位
	capacity_label.offset_right = -20
	capacity_label.offset_bottom = 950
	# 关闭按钮放右上角
