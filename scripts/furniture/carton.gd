...
@export var item_type: String = "carton"  # 物品类型（如桌子是家具）
...
var surface_z_index # 获取当前物品增加的层级
var item_direction = "front" # 记录当前物品的方向
var is_functional := true  # 当前功能是否启用
var ac = false # 用来辅助，使物品可以三叠
var item_data: Dictionary = { "id": "carton", "name": "箱子", "price": 1,
"icon": preload("res://art_resource/Fewobject/carton_up.png") }

func _ready():
	var collision = $CollisionShape2D
	add_to_group("dragers")
	set_meta("item_type", "carton")
	...
	if event is InputEventMouseButton:
		...
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			...
			if not is_being_dragged:
				...
				if can_place and judge_surface() and judge_wall_furniture_place() and not is_other_item:
...
func _process(_delta):
	...
	if is_being_dragged:
		...
		if is_functional:
			detect_surface()
	if ac and is_overlapping:
		is_other_item = false
	elif ac and not is_overlapping:
		is_other_item = true
...
func joint_diast_item():
	if (is_being_dragged or (overlapping_furniture != null and overlapping_furniture.is_being_dragged)) and not is_been_dragged:
		...
		if overlapping_furniture == null or (overlapping_furniture != null and not overlapping_furniture.is_been_dragged):
			for area in sur:
...
func replace_with_rotated_version(new_direction):
	var scene_path := "res://scenes/furnitures/carton_%s.tscn" % new_direction
...
func judge_surface():
	...
	for i in overlap_surfaces.size():
		...
		if other.has_meta("item_type") and other.get_meta("item_type") == "furniture" and (not other.is_being_dragged or overlapping_furniture == other):
...
func judge_put():
	for area in overlap_put:
		...
		if item.has_meta("item_type") and (item.get_meta("item_type") == "object" or item.get_meta("item_type") == "surface_mat"):
			if item.get_parent().get_parent() != null and item.get_parent().get_parent() == self:
	...
	if is_overlapping:
		for area in overlap_put:
			var other = area.get_parent()
			if other.has_meta("item_type") and (other.get_meta("item_type") == "object" or other.get_meta("item_type") == "carton"):
				if not is_same_object(area) and other.is_overlapping:
					return false
		return true
	for area in overlap_put:
		var item = area.get_parent()
		if item.has_meta("item_type"):
			if item.get_meta("item_type") == "object" and not item.is_overlapping:
				return false
			if item.get_meta("item_type") == "carton" and not item.is_overlapping:
				return false
			if item.get_meta("item_type") == "furniture":
				return false
	return true
...
func judge_collison():
	var collision = $CollisionShape2D
	# 自动禁用碰撞体（放在桌面上时）
	collision.disabled = is_overlapping
		...
		if not is_same_object(area):
			...
			if parent.is_in_group("dragers"):  # 你的物品都在这个组
				if parent.has_meta("item_type") and parent.get_meta("item_type") == "surface_mat":
					if parent.can_place and parent.overlapping_furniture == self:
						return true  # 检测到物体
...
func get_surface_z_index():
	if overlapping_furniture and is_overlapping:
		...
		var z_index_offset = 10 / (oavr_marker.global_position.y - $Sprite2D/ShelterBase.global_position.y) 
...
func _on_surface_area_entered(area):
	...
	var item = area.get_parent()
	# 若进入的区域也是可放置物品的区域
	if area.is_in_group("placeable_surface"):  # 放置物品加个 group
		if item.has_meta("item_type") and item.get_meta("item_type") == "furniture":
			if has_items_on_top():
				ac = true
				is_other_item = true
