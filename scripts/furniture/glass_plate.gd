...
var item_data: Dictionary = { "id": "glass_plate", "name": "玻璃杯", "price": 1,
"icon": preload("res://art_resource/Fewobject/Glass plate.png") }

func _ready():
	var collision = $CollisionShape2D
	add_to_group("dragers")
	add_to_group("glass_plate")
...
func check_for_stack():
	for other in get_tree().get_nodes_in_group("glass_plate"):
		if other == self:
			continue
		var my_center = self.global_position
		var other_center = other.global_position
		var distance = my_center.distance_to(other_center)
		# 可调参数，越小越严格（16 像素内）
		if distance < 16.0:
			stack_with(other)
			break
...
# 消除原有，生成新的叠放的场景
func stack_with(other):
	var marker_pos = other.get_node("Positioning").global_position
	print("发现两个杯子重叠，生成新场景")
	var double_gplate_scene = preload("res://scenes/furnitures/double_gplate.tscn")
	var double_gplate = double_gplate_scene.instantiate()
	# 添加到主场景
	var world = get_tree().get_first_node_in_group("editable").get_node("World")  # 你自己的世界根节点
	world.add_child(double_gplate)
	# 设置新叠加物体的位置为底部杯子的位置
	var new_marker = double_gplate.get_node("Positioning")
	var offset_pos = new_marker.position * double_gplate.global_scale
	double_gplate.global_position = marker_pos - offset_pos
	DragManager.clear_if_hovered(self)
	DragManager.clear_if_hovered(other)
	# 删除原杯子
	self.queue_free()
	other.queue_free()
