...
@onready var shelves11 := $Shelves11
@onready var shelves12 := $Shelves12
@onready var shelves13 := $Shelves13
@onready var shelves14 := $Shelves14
@onready var shelves15 := $Shelves15
...
var depot_amount : int # 仓库存储的物品数量
var inventory_id : int
var count : int # 仓库中的物品数量
var item_data: Dictionary = { "id": "shelves_1", "name": "仓库架",
"price": 10, "icon": preload("res://art_resource/thing/Seats1_blue_icon.png") }
...
func _ready():
	...
	call_deferred("update_count")

# 获取自身id
func get_id():
	inventory_id = DepotManager.iron_shelf
	DepotManager.iron_shelf += 1
	print(inventory_id)
...
func _process(_delta):
	...
	if is_being_dragged:
		...
		z_index = get_surface_z_index()
		if depot_ui and depot_ui.visible and $depotArea.player_in_range:
			count_depot()

# 更新加载数量
func update_count():
	if inventory_id:
		DepotManager.load_depot_items(inventory_id)
		count_depot()

# 根据仓库物品数量来显示
func count_depot():
	count = DepotManager.count_item()
	if count == 0:
		shelves11.visible = false
	if count > 0:
		shelves11.visible = true
		shelves12.visible = false
	if count > 2:
		shelves12.visible = true
		shelves13.visible = false
	if count > 4:
		shelves13.visible = true
		shelves14.visible = false
	if count > 6:
		shelves14.visible = true
		shelves15.visible = false
	if count > 8:
		shelves15.visible = true
