...
var item_data: Dictionary = { "id": "refrigerator", "name": "冰箱",
"price": 5, "icon": preload("res://art_resource/thing/Seats1_blue_icon.png") }

func _ready():
	@warning_ignore("shadowed_variable")
	var collision = $CollisionShape2D
	add_to_group("dragers")
	add_to_group("refrigerator")
	...
	#$RefrigertorFace1.area_exited.connect(_on_surface_area_exited)
	#$RefrigertorFace2.area_exited.connect(_on_surface_area_exited)
	collision.disabled = false
	$OutlineSprite.visible = false
	refrigerator_open.visible = false
	...
	if event is InputEventMouseButton:
		...
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			...
			if not is_being_dragged:
				...
				if can_place and judge_put() and judge_wall_furniture_place():
...
# 检测桌面区域的物品
func detect_surface():
	var surface_area1 = get_node_or_null("RefrigertorFace1")
	var surface_area2 = get_node_or_null("RefrigertorFace2")
	if not surface_area1 or not surface_area2:
		return false
		...
		if not is_same_object(area):
			...
			if not parent in surface_item and parent.get_parent().name != "StackContainer":
				if parent.is_in_group("dragers"):  # 你的物品都在这个组
					if parent.has_meta("item_type") and parent.get_meta("item_type") == "sticker":
						if parent.overlapping_furniture == self:
							surface_item.append(parent)
	for area in surface_area2.get_overlapping_areas():
		if not is_same_object(area):
			var parent = area.get_parent()
			if not parent in surface_item and parent.get_parent().name != "StackContainer":
				if parent.is_in_group("dragers"):  # 你的物品都在这个组
...
func replace_with_rotated_version(new_direction):
	var scene_path = "res://scenes/furnitures/special/refrigerator_%s.tscn" % new_direction
...
func has_items_on_top() -> bool:
	var stack_container = get_node_or_null("StackContainer")
	if stack_container.get_child_count() > 0 or surface_item != []:
		return true  # 检测到物体
	return false
...
# 开关冰箱函数
func open_refrigerator():
	refrigerator_open.play("open")
	refrigerator_open.z_index = 10
	refrigerator_open.visible = true
	is_occupy = true

func close_refrigerator():
	refrigerator_open.visible = false
	is_occupy = false
