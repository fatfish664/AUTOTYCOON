...
var item_data: Dictionary = { "id": "bed", "name": "床", "price": 70, "icon": preload("res://art_resource/changjing/bed.png") }
		...
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			...
			if not is_being_dragged:
				...
				if is_area and DragManager.start_drag(self):
					...
					drag_offset = position - mouse_local
					z_index = 2100
...
func _process(_delta):
	...
	if is_being_dragged:
		...
		position = parent.to_local(get_global_mouse_position()) + drag_offset
		#var mouse_pos = parent.to_local(get_global_mouse_position()) + drag_offset
		## 直接应用边缘吸附
		#var snapped_pos = apply_edge_snap(mouse_pos)
...
func _check_placeable_area():
	...
	if can_place:
		...
		sprite.modulate = Color(1, 1, 1, 0.5)  # 红色+半透明（非法放置）

#func apply_edge_snap(pos: Vector2) -> Vector2:
	#var shape = put_place.get_node("CollisionShape2D")
	#print("1")
	#if shape == null or not shape.shape is RectangleShape2D:
		#return pos
	#var parent = get_parent()
	#var this_size = shape.shape.extents * 2
	#var this_rect = Rect2(pos - this_size * 0.5, this_size)
	#for area in get_tree().get_nodes_in_group("placeable_floor"):
		#print("2")
		#if not area is Area2D:
			#continue
		#var a_shape = area.get_node("CollisionShape2D")
		#if a_shape == null or not a_shape.shape is RectangleShape2D:
			#continue
		##var area_size = a_shape.shape.extents * 2 * area.scale
		##var area_pos = a_shape.get_transform().origin - area_size * 0.5
		##var area_rect = Rect2(area_pos, area_size)
		## 将区域中心坐标从全局转为当前父节点的局部坐标空间
		#var area_center_global = a_shape.get_global_transform().origin
		#var area_center_local = parent.to_local(area_center_global)
		## 使用未缩放版本（原始 extents），乘以自身 scale（不乘 global_scale）
		#var area_size = a_shape.shape.extents * 2 * area.scale
		#var area_rect = Rect2(area_center_local - area_size * 0.5, area_size)
		#print("3")
		## 左边缘吸附
		#if abs(this_rect.position.x - area_rect.position.x) <= SNAP_MARGIN:
			#print("➡️ 左边缘差值: ", abs(this_rect.position.x - area_rect.position.x))
			#pos.x = area_rect.position.x + this_size.x / 2
		## 右边缘吸附
		#elif abs(this_rect.end.x - area_rect.end.x) <= SNAP_MARGIN:
			#pos.x = area_rect.end.x - this_size.x / 2
			#print("➡️ 右边缘差值: ", abs(this_rect.end.x - area_rect.end.x))
		## 顶部边缘吸附
		#if abs(this_rect.position.y - area_rect.position.y) <= SNAP_MARGIN:
			#print("➡️ 顶部边缘差值: ", abs(this_rect.position.y - area_rect.position.y))
			#pos.y = area_rect.position.y + this_size.y / 2
		## 底部边缘吸附
		#elif abs(this_rect.end.y - area_rect.end.y) <= SNAP_MARGIN:
			#print("➡️ 底部边缘差值: ", abs(this_rect.end.y - area_rect.end.y))
			#pos.y = area_rect.end.y - this_size.y / 2
	#return pos
