...
@onready var sofa_hide := $SofaHide
...
var item_data: Dictionary = { "id": "sofa", "name": "沙发", "price": 10, 
"icon": preload("res://art_resource/thing/Seats1_blue_icon.png") }
...
func replace_with_rotated_version(new_direction):
	var tem_direction = new_direction
	if new_direction == "left":
		tem_direction = "right"
	var scene_path := "res://scenes/furnitures/sofa_%s.tscn" % tem_direction
	...
	if new_direction == "left":
		new_scene.sprite.scale.x = -1
		new_scene.outline_sprite.scale.x = -1
		new_scene.sit_area.scale.x = -1
		new_scene.sofa_hide.scale.x = -1
		new_scene.marker.scale.x = -1
...
# 坐下/起身函数
func sit_down():
	sofa_hide.visible = true
	# 这样设置的sofa_hide的层级会加上物品本身的层级，是一个变化的值，
	# 可以覆盖到座位上的角色又不会覆盖到站到沙发后面的角色（原理未知）
	sofa_hide.z_index = 40
	is_occupy = true

func stand_up():
	sofa_hide.z_index = 0
	sofa_hide.visible = false
	is_occupy = false
