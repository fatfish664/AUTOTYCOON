...
@onready var upper_area := $UpperClickArea
...
@onready var gplate_sprite = $GplateSide # 用来实现触碰描边
...
var upper_gplate = false # 是否准备拆除
var is_functional := true  # 当前功能是否启用
var item_data: Dictionary =  { "id": "double_gplate", "name": "叠放杯", "price": 2, "icon": preload("res://art_resource/Fewobject/Glass plate_pile up.png") }
...
func _ready():
	...
	upper_area.mouse_entered.connect(_on_upper_area_entered)
	upper_area.mouse_exited.connect(_on_upper_area_exited)
...
func disable_all_functions():
	...
	if has_node("UpperClickArea"):
		$UpperClickArea.set_deferred("monitoring", false)
		$UpperClickArea.set_deferred("mouse_filter", Control.MOUSE_FILTER_IGNORE)
...
func enable_all_functions():
	...
	if has_node("PutPlace"):
		...
		$PutPlace.set_deferred("mouse_filter", Control.MOUSE_FILTER_IGNORE)
	if has_node("UpperClickArea"):
		$UpperClickArea.set_deferred("monitoring", true)
		$UpperClickArea.set_deferred("mouse_filter", Control.MOUSE_FILTER_IGNORE)

# 使物品拆分
func split_into_two():
	var marker_pos = $Positioning.global_position
	# 加载玻璃杯预制体
	var glass_plate_scene = ResourceLoader.load("res://scenes/furnitures/glass_plate.tscn")
	# 实例化第一个杯子（固定）
	var gplate1 = glass_plate_scene.instantiate()
	# 实例化第二个杯子（拖拽）
	var gplate2 = glass_plate_scene.instantiate()
	# 添加到主世界场景中
	var world := get_tree().get_first_node_in_group("editable").get_node("World")
	world.add_child(gplate1)
	world.add_child(gplate2)
	# 赋予位置
	# 获取新叠放场景中 Marker2D 的偏移（局部位置）
	var gplate1_marker = gplate1.get_node("Positioning")
	var offset_pos = gplate1_marker.position * gplate1.global_scale
	gplate1.global_position = marker_pos - offset_pos
	gplate2.global_position =get_global_mouse_position()
	# 触发拖拽（需要实现此方法）
	gplate2.first_ex_drag()
...
func return_to_backpack():
	...
	# 通知背包系统回收此物品
	backpack.return_item("glass_plate")
	backpack.return_item("glass_plate")
...
func get_surface_z_index():
	if overlapping_furniture and is_overlapping:
		...
		var z_index_offset = abs(10 / (over_marker.global_position.y - $Sprite2D/ShelterBase.global_position.y))
...
func _on_upper_area_entered():
	upper_gplate = true
	if DragManager.is_edit_mode and not is_being_dragged and DragManager.active_item == null and outline_sprite.visible:
		clear_outline()
		gplate_sprite.visible = true

func _on_upper_area_exited():
	upper_gplate = false
	if DragManager.is_edit_mode and not is_being_dragged and DragManager.active_item == null:
		if gplate_sprite.visible:
			apply_outline()
		gplate_sprite.visible = false
		if not is_area:
			clear_outline()
	else:
		gplate_sprite.visible = false
