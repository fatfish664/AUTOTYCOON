...
@onready var pit_no_car := $Pit1LeftNocar
@onready var pit_have_car := $Pit1LeftHavecar
@onready var car_shadow := $CarShadow
@onready var car_tire := $CarTire
@onready var car_tire2 := $CarTire2
@onready var car := $Car1
@onready var pit_left := $Pit1Left1
...
@onready var collision2 = $CollisionShape2D2
@onready var car_collision = $CarCollisionShape2D
...
var exist_car = false  # 是否存在车
var is_occupy = false # 是否在被使用
var change_seat  # 随物品在不同的区域进行转换(其他脚本有判定，不能删)
var item_data: Dictionary = { "id": "work_place",
"name": "工作位", "price": 10, "icon": preload("res://art_resource/thing/Seats1_blue_icon.png") }

func _ready():
	add_to_group("dragers")
	add_to_group("work_place")
...
func _process(_delta):
	...
	if is_being_dragged:
		...
		$CarCollisionShape2D.disabled = true
		if can_place and judge_put() and judge_wall_furniture_place():
		...
		z_index = get_surface_z_index()
		pit_left.z_index = $Pit1Left1/ShelterBase.global_position.y
...
# 改变碰撞体积
func change_collision(a):
	$CarCollisionShape2D.disabled = not a

# 转换当前的模式（是否有车）
func change_mode():
	exist_car = not exist_car
	pit_have_car.visible = exist_car
	car_shadow.visible = exist_car
	car_tire.visible = exist_car
	car_tire2.visible = exist_car
	car.visible = exist_car
	pit_no_car.visible = not exist_car
	change_collision(exist_car)
